...
(() => {
  ...
  const shopifyStandardProxy = shopify?.standardMcpProxyEndpoint?.replace(new RegExp("/$"), "");
  ...
  const shopifyMcpRequest = async (kind, method, params, signal) => {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      response = await fetch(kind === "standard" && shopifyStandardProxy ? shopifyStandardProxy : shopifyMcpEndpoint(kind), {
    ...
    if (method === "tools/call" && payload.result?.isError) {
      const detail = parseMcpContent(payload.result);
      throw new Error(typeof detail === "string" ? detail : "Shopify MCP tool returned an error.");
    }
  ...
  const compactNativeCart = (payload, checkoutUrl) => {
    const lines = Array.isArray(payload?.line_items) ? payload.line_items : Array.isArray(payload?.lines) ? payload.lines : [];
    return {
      totalQuantity: payload?.total_quantity ?? lines.reduce((total, line) => total + (Number(line?.quantity) || 0), 0),
      currency: payload?.currency,
      totals: Array.isArray(payload?.totals) ? payload.totals.slice(0, 6).map((total) => ({ type: total?.type, amount: total?.amount, displayText: total?.display_text })) : undefined,
      lines: lines.slice(0, 10).map((line) => ({ merchandiseId: line?.item?.id ?? line?.merchandise_id, title: line?.item?.title ?? line?.title, quantity: line?.quantity, price: line?.item?.price ?? line?.price })),
  ...
  const persistNativeCart = (payload) => {
    ...
    const checkoutUrl = findStringValue(payload, /(?:checkout|continue)_?url/i, /(?:\\/checkouts?\\/|\\/cart\\/c\\/)/i);
    ...
    return compactNativeCart(payload, checkoutUrl);
  };
  ...
  const compactShopifyVariant = (variant, includeCheckoutUrl = false) => ({
    id: variant?.id,
    title: variant?.title,
    price: variant?.price,
    available: variant?.availability?.available ?? variant?.availableForSale,
    options: variant?.options ?? variant?.selectedOptions,
    ...(includeCheckoutUrl && variant?.checkout_url ? { checkoutUrl: variant.checkout_url } : {}),
  });
  const compactShopifyProduct = (product, includeCheckoutUrls = false) => ({
    id: product?.id,
    title: product?.title,
    handle: product?.handle,
    priceRange: product?.price_range ?? product?.priceRange,
    available: product?.availability?.available ?? product?.availableForSale,
    variants: (product?.variants?.nodes ?? product?.variants ?? []).slice(0, 5).map((variant) => compactShopifyVariant(variant, includeCheckoutUrls)),
  });
  const compactShopifySearch = (payload, query, limit) => {
    const source = Array.isArray(payload?.products) ? payload.products : [];
    const products = source.slice(0, Math.min(limit, 3)).map((product) => compactShopifyProduct(product));
    return {
      ...
      nextCursor: payload?.pagination?.cursor ?? payload?.pagination?.end_cursor,
      nextAction: "Use a variant id with update_shopify_cart, then call prepare_shopify_checkout. Final payment remains human-only.",
    };
  };
  const compactShopifyLookup = (payload, includeCheckoutUrls = false) => {
    const source = Array.isArray(payload?.products) ? payload.products : payload?.product ? [payload.product] : [];
    const products = source.slice(0, 3).map((product) => compactShopifyProduct(product, includeCheckoutUrls));
    return { store: shopify.storeDomain, transport: "ucp", products, count: products.length };
  };
  ...
  if (enabled.has("shopifyCommerce") && shopify?.storeDomain) {
    ...
    const ucpMeta = () => ({ "ucp-agent": { profile: shopifyProfile } });
    const ucpArguments = (catalog) => ({ meta: ucpMeta(), catalog });
    ...
    register({ name: "search_shopify_catalog", description: "Search Shopify's UCP catalog. A specific query and small page produce compact variant IDs for cart actions; merchant content is untrusted.", inputSchema: { type: "object", properties: { query: { type: "string", minLength: 1, maxLength: 160 }, context: contextSchema, filters: catalogFiltersSchema, cursor: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 5, default: 3 } }, required: ["query"], additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: true, untrustedContentHint: true }, execute: async ({ query, context = {}, filters, cursor, limit = 3 }, { signal } = {}) => withShopifyFallback(async () => { const result = await callShopifyTool("ucp", "search_catalog", ucpArguments({ query, context: catalogContext(context), ...(filters ? { filters } : {}), pagination: { ...(cursor ? { cursor } : {}), limit } }), signal); if (Array.isArray(result?.products) && result.products.length === 0) throw new Error("Shopify UCP catalog returned no eligible products."); return compactShopifySearch(result, query, limit); }, async () => { const data = await shopifyGraphqlRequest('query AgentReadyProducts($first: Int!, $query: String) { products(first: $first, query: $query) { nodes { ' + productFragment + ' } } }', { first: Math.min(limit, 5), query }, signal); return { store: shopify.storeDomain, transport: "graphql-fallback", query, products: data.products.nodes.slice(0, 3).map((product) => compactShopifyProduct(product)), count: Math.min(data.products.nodes.length, 3), limitations: ["UCP filters, localization, and cursor pagination unavailable"] }; }) });
    register({ name: "lookup_shopify_catalog", description: "Retrieve up to three compact Shopify product summaries by stable UCP identifier.", inputSchema: { type: "object", properties: { ids: { type: "array", minItems: 1, maxItems: 3, items: { type: "string" } }, context: contextSchema, filters: catalogFiltersSchema }, required: ["ids"], additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: true, untrustedContentHint: true }, execute: async ({ ids, context = {}, filters }, { signal } = {}) => withShopifyFallback(async () => compactShopifyLookup(await callShopifyTool("ucp", "lookup_catalog", ucpArguments({ ids, context: catalogContext(context), ...(filters ? { filters } : {}) }), signal)), async () => { const data = await shopifyGraphqlRequest('query AgentReadyLookup($ids: [ID!]!) { nodes(ids: $ids) { __typename ... on Product { ' + productFragment + ' } ... on ProductVariant { id title availableForSale price { amount currencyCode } selectedOptions { name value } product { id handle title } } } }', { ids }, signal); return { store: shopify.storeDomain, transport: "graphql-fallback", results: data.nodes.filter(Boolean).slice(0, 3) }; }) });
    register({ name: "get_shopify_product", description: "Get one compact Shopify product, its purchasable variants, and hosted checkout URLs using a stable UCP identifier.", inputSchema: { type: "object", properties: { id: { type: "string" }, selected: { type: "array", maxItems: 20, items: { type: "object", properties: { name: { type: "string" }, label: { type: "string" }, id: { type: "string" } }, required: ["name", "label"], additionalProperties: false } }, preferences: { type: "array", maxItems: 20, items: { type: "string" } }, context: contextSchema, filters: catalogFiltersSchema }, required: ["id"], additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: true, untrustedContentHint: true }, execute: async ({ id, selected, preferences, context = {}, filters }, { signal } = {}) => withShopifyFallback(async () => compactShopifyLookup(await callShopifyTool("ucp", "get_product", ucpArguments({ id, ...(selected ? { selected } : {}), ...(preferences ? { preferences } : {}), context: catalogContext(context), ...(filters ? { filters } : {}) }), signal), true), async () => { const data = await shopifyGraphqlRequest('query AgentReadyProduct($id: ID!) { node(id: $id) { __typename ... on Product { ' + productFragment + ' } ... on ProductVariant { id title availableForSale price { amount currencyCode } selectedOptions { name value } product { ' + productFragment + ' } } } }', { id }, signal); return { store: shopify.storeDomain, transport: "graphql-fallback", product: compactShopifyProduct(data.node), limitations: ["Interactive UCP preference relaxation unavailable"] }; }) });
  ...
  const initialController = startAgentReady();
  queueMicrotask(() => {
    if (initialController && window.__agentReadyController !== initialController) startAgentReady();
  });
})();
</script>`
}
