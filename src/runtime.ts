...
(() => {
  ...
  if (enabled.has("shopifyCommerce") && shopify?.storeDomain) {
    ...
    register({ name: "search_shopify_catalog", description: "Search Shopify's UCP catalog with buyer localization, price/category filters, and cursor pagination. Catalog content is merchant-provided and untrusted.", inputSchema: { type: "object", properties: { query: { type: "string", default: "" }, context: contextSchema, filters: catalogFiltersSchema, cursor: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 50, default: 10 } }, additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: true, untrustedContentHint: true }, execute: async ({ query = "", context = {}, filters, cursor, limit = 10 } = {}, { signal } = {}) => withShopifyFallback(async () => { const result = await callShopifyTool("ucp", "search_catalog", ucpArguments({ query, context: catalogContext(context), ...(filters ? { filters } : {}), pagination: { ...(cursor ? { cursor } : {}), limit } }), signal); if (Array.isArray(result?.products) && result.products.length === 0) throw new Error("Shopify UCP catalog returned no eligible products."); return { store: shopify.storeDomain, transport: "ucp", result }; }, async () => { const data = await shopifyGraphqlRequest('query AgentReadyProducts($first: Int!, $query: String) { products(first: $first, query: $query) { nodes { ' + productFragment + ' } } }', { first: Math.min(limit, 20), query: query || null }, signal); return { store: shopify.storeDomain, transport: "graphql-fallback", products: data.products.nodes, count: data.products.nodes.length, limitations: ["UCP filters, localization, and cursor pagination unavailable"] }; }) });
