...
(() => {
  ...
  const shopifyRequest = async (query, variables = {}, signal) => {
  ...
  const getShopifyCart = async (signal) => {
    const id = sessionStorage.getItem(shopifyCartKey); if (!id) return null;
    const data = await shopifyRequest('query AgentReadyCart($id: ID!) { cart(id: $id) { ...AgentReadyCart } } ' + cartFragment, { id }, signal);
