...
For proxied Framer responses, the Worker also adds `Permissions-Policy: tools=(self)` and `Origin-Agent-Cluster: ?1`, matching the current WebMCP secure-context and origin-keyed-agent-cluster requirements. Route this Worker consistently across the custom origin; do not create paths with conflicting `Origin-Agent-Cluster` policies.
