...
The live demo is itself a responsive Framer site created and edited through a Framer MCP server. The repository includes isolated tests for all 30 tools and an end-to-end test for the published Custom Code. Shopify's native MCP/UCP endpoints provide catalog, policy, and cart semantics, with GraphQL fallback when a store restricts MCP. Payment boundaries are explicit: AgentReady can assemble a Shopify cart and prepare checkout, while Shopify owns retail payment authentication; Cloudflare Workers demonstrate agent-native HTTP 402 payment challenges, paid-crawl pricing, AI Search knowledge, anonymous tool analytics, and Browser Run readiness verification.
...
Briefly show the open-source runtime, narrow schemas, payment-secret guard, Shopify MCP/UCP discovery and cart redaction, Cloudflare Workers, and passing 30-tool test.
...
- Execution: a working Framer plugin, deployed demo, 30 optional tools, runtime tests, live E2E test, Cloudflare Workers, and CI.
