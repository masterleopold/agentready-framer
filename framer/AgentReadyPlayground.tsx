...
export default function AgentReadyPlayground({ checkoutUrl = "", creatorPrice = "$49", studioPrice = "$149", agencyPrice = "$399", paymentEndpoint = "", intelligenceEndpoint = "" }: { checkoutUrl?: string; creatorPrice?: string; studioPrice?: string; agencyPrice?: string; paymentEndpoint?: string; intelligenceEndpoint?: string }) {
  ...
  return <div style={{ width: "100%", padding: 24, color: "#f5f5f5", background: "#0b0b0b", border: "1px solid #242424", borderRadius: 20, fontFamily: "Inter, sans-serif", boxSizing: "border-box" }}>
    ...
    <form data-agentready-form="plugin-purchase" aria-label="Purchase AgentReady plugin license" onSubmit={purchase} onChange={(event) => { const target = event.target as HTMLInputElement; if (target.name === "license") setLicense(target.value) }} style={{ marginTop: 14, padding: 18, border: "1px solid #292929", borderRadius: 14, background: "#111" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 20, marginBottom: 16 }}><div><div style={{ color: "#0099ff", fontSize: 10, fontWeight: 700, letterSpacing: ".12em" }}>BUY AGENTREADY WITH WEBMCP</div><h3 style={{ margin: "8px 0 4px", fontSize: 22, fontWeight: 500 }}>Purchase the plugin using its own tools.</h3><p style={{ margin: 0, color: "#777", fontSize: 11 }}>The agent prepares the order. You review and complete secure checkout.</p></div><strong style={{ fontSize: 22, whiteSpace: "nowrap" }}>{selectedPrice} <small style={{ color: "#777", fontSize: 10 }}>ONE-TIME</small></strong></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 8, marginTop: 12 }}>
        <Choice type="radio" name="license" value="creator" defaultChecked>Creator · 1 workspace · {creatorPrice}</Choice>
        <Choice type="radio" name="license" value="studio">Studio · 5 workspaces · {studioPrice}</Choice>
        <Choice type="radio" name="license" value="agency">Agency · unlimited sites · {agencyPrice}</Choice>
      </div>
      <label style={{ display: "flex", alignItems: "center", gap: 9, marginTop: 12, color: "#999", fontSize: 11 }}><input required type="checkbox" name="acceptTerms" value="accepted" style={{ accentColor: "#0099ff" }} />I will review the license and refund terms before final payment.</label>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 14, flexWrap: "wrap" }}><button type="submit" style={button}>{checkoutUrl ? "Continue to secure checkout →" : "Prepare checkout →"}</button><span aria-live="polite" style={{ color: purchaseReady ? "#69e6b7" : "#777", fontSize: 11 }}>{purchaseReady ? "✓ " + license + " order prepared · Final payment remains human-controlled" : "Card and wallet credentials are never exposed to WebMCP."}</span></div>
    </form>

    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 14 }}>
      <form data-agentready-form="application" aria-label="Multi-step application" onSubmit={(event) => { event.preventDefault(); setApplicationStatus("✓ Application received in this local demo") }} style={{ padding: 18, border: "1px solid #292929", borderRadius: 14, background: "#111" }}>
        ...
        <div aria-hidden={step !== 2} style={{ display: step === 2 ? "grid" : "none", gap: 12 }}>
          ...
          <label style={label}>REFERENCE FILES<input style={{ ...field, paddingTop: 10 }} type="file" name="referenceFiles" accept=".pdf,.png,.jpg" multiple /></label>
        </div>
        <div aria-hidden={step !== 3} style={{ display: step === 3 ? "block" : "none", padding: 16, color: "#aaa", background: "#181818", borderRadius: 10, fontSize: 12, lineHeight: 1.55 }}>The agent prepared your details. Review every value before the final submission. Payment and authentication controls always remain yours.</div>
        ...
        {applicationStatus && <p role="status" style={{ margin: "12px 0 0", color: "#69e6b7", fontSize: 11 }}>{applicationStatus}</p>}
      </form>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14, marginTop: 14 }}>
      <section aria-label="Shopify cart demo" style={{ padding: 18, border: "1px solid #292929", borderRadius: 14, background: "#111" }}><div style={{ color: "#77d89c", fontSize: 10, fontWeight: 700, letterSpacing: ".12em" }}>SHOPIFY STOREFRONT MCP + UCP</div><h3 style={{ margin: "9px 0 5px", fontSize: 18, fontWeight: 500 }}>Agent Kit — Pro</h3><p style={{ margin: "0 0 14px", color: "#777", fontSize: 11 }}>Localized catalog discovery, variant selection, merchant policies, cart state, and safe hosted checkout.</p><button type="button" style={button} onClick={() => setCart((value) => value + 1)}>Add to cart · ${"29"}</button><span style={{ marginLeft: 10, color: "#888", fontSize: 11 }}>{cart} in cart</span></section>
