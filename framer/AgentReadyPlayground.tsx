...
function Choice({ name, value, children, type = "checkbox" }: { name: string; value: string; children: React.ReactNode; type?: "checkbox" | "radio" }) {
  return <label style={{ display: "flex", alignItems: "center", gap: 9, padding: "11px 12px", color: "#ddd", border: "1px solid #2b2b2b", borderRadius: 9, font: "500 12px Inter, sans-serif" }}>
    <input type={type} name={name} value={value} style={{ accentColor: "#0099ff" }} />{children}
  </label>
}

export default function AgentReadyPlayground() {
